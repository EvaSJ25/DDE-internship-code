using LinearAlgebra
function track_curve(userf,y0,ytan; h=1e-5,userdf=x->jacobian(userf,x;h=h), nmax=100,stepsize=0.01,tol=1e-8,maxit=6)
    ##inputs:
    ...
    #h is the stepsize for use in jacobian
    #tol is the tolerance used in newton
    #maxit are the maximum number of iterations newton is allowed to do
