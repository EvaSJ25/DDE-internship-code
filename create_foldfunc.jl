...
function create_foldfunc(f_DDE, f_tau,pars,x0,p0,par_indx,nd;m=100)
    ...
    stabfunc=stab_func(f_DDE, f_tau,x0,p0,params,par_indx,nd)
    eigvals=stabfunc[2]
    eigvecs=stabfunc[3]

    fold_indx=argmin(abs.(eigvals))#finds index of the eigenvalue that is closest to being purely imaginary
    lamini=eigvals[fold_indx]
    ...
    #y0=vcat(x0,vrini,viini,lamini,p0) #do i need lamini???
    ...
    function ffold(y)
        ...
        if length(par_indx)==1
            ...
            u,vr,vi,p=y[1:n], y[n+1:2*n], y[2*n+1:3*n],y[3*n+1:end] #u,vr,vi,om,p=y[1:2], y[3:4], y[5:6],y[7],y[8]
        end
