...
function create_ststfunc(f_DDE, u0, pars,nd; par_indx=[])
    ...
    #par_indx is the index of the parameter the user wishes to vary (to do continuation over)
    ...
    #f_equilibrium is a function that is used to find the equilibrium (finds f(u,u,..,u)=0)
    ...
    y0=vcat(u0,pars[par_indx]) #n+1 dimensional vector of initial guess for equilibirum information (u0 with the n+1th term being the varied parameter)
