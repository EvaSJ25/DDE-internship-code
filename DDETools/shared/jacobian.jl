function jacobian(f,x; h=1e-5)
    ...
    for j in 1:n
        ...
        ej[j]=1 #sets jth component to 1 (all the others are zero) - jth basis vector

        J[:,j]=(1/2h)*(f(x+(h*ej)) - f(x-(h*ej))) #differentiates function f with respect to x j-th component using the finite difference method
    end
